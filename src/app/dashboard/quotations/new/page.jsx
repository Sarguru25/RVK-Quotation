...
`,
    ...
    async function fetchQuote() {
      try {
        ...
        setForm({
          ...
          currency_code: fullQuote.currency_code || fullQuote.rawZohoData?.currency_code || "INR",
  ...
  const formatCurrency = (amount, code = "INR") =>
    new Intl.NumberFormat("en-IN", { style: "currency", currency: code }).format(amount || 0);
          ...
          <div className="w-full min-h-full relative">
            ...
            <div className="max-w-[1200px] mx-auto py-8 px-6 pb-40">
              <div className="grid grid-cols-1 md:grid-cols-12 gap-y-6 gap-x-8 mb-12">
                  ...
                  <SearchableSelect
                    ...
                    onChange={(val) => {
                      ...
                      setForm((prev) => ({
                        ...
                        currency_code: selected?.currency_code || selected?.rawZohoData?.currency_code || "INR",
                ...
                <div className="md:col-span-3 flex items-center md:justify-end"><label className="text-sm font-medium text-red-500">Estimated Margin *</label></div>
                <div className="md:col-span-3">
                  <div className="flex items-center border border-gray-300 rounded-md bg-white">
                    <input type="text" name="estimated_margin" value={form.estimated_margin || ""} onChange={handleChange} className="w-full border-none rounded-l-md text-sm px-3 py-2 outline-none focus:ring-0" />
                    <span className="bg-gray-50 text-gray-500 px-3 py-2 border-l border-gray-300 text-sm">%</span>
                  </div>
                </div>
                      ...
                      return (
                        <tr key={index} className="hover:bg-gray-50 transition-colors group">
                          ...
                          <td className="px-5 py-3 align-top">
                            ...
                            {lineTax > 0 && <div className="text-xs text-green-600 mt-1 text-right">+{formatCurrency(lineTax, form.currency_code || "INR")}</div>}
                          </td>
                          <td className="px-5 py-3 align-top text-right text-sm text-gray-800 font-semibold pt-4">
                            {formatCurrency(lineAmount, form.currency_code || "INR")}
                          </td>
              ...
              <div className="flex flex-col md:flex-row justify-between mt-8">
                ...
                <div className="w-full md:w-[400px] bg-gray-50 p-6 rounded-lg border border-gray-200 shadow-sm h-fit">
                  <div className="flex justify-between items-center mb-5 text-sm">
                    ...
                    <span className="font-semibold text-gray-900">{formatCurrency(subTotal, form.currency_code || "INR")}</span>
                  </div>
                  <div className="flex justify-between items-center mb-5 text-sm">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-red-500 font-medium w-24 text-right">-{formatCurrency(discountAmount, form.currency_code || "INR")}</span>
                    </div>
                  </div>
                  {taxTotal > 0 && (
                    <div className="flex justify-between items-center mb-5 text-sm">
                      <span className="text-gray-600">Tax</span>
                      <span className="text-green-600 font-medium">+{formatCurrency(taxTotal, form.currency_code || "INR")}</span>
                    </div>
                  )}
                  <div className="flex justify-between items-center mb-5 text-sm">
                    <span className="text-gray-600">Shipping Charges</span>
                    <div className="flex items-center gap-3">
                      <input type="number" step="any" value={form.shipping_charges || ""} onChange={e => setForm(prev => ({ ...prev, shipping_charges: e.target.value }))} className="w-24 border border-gray-300 rounded text-right px-3 py-1.5 text-sm outline-none bg-white" placeholder="0.00" />
                      <span className="text-gray-900 font-medium w-20 text-right">{formatCurrency(shippingCharges, form.currency_code || "INR")}</span>
                    </div>
                  </div>
                  <div className="flex justify-between items-center mb-5 text-sm">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-gray-900 font-medium w-20 text-right">{formatCurrency(adjustment, form.currency_code || "INR")}</span>
                    </div>
                  </div>
                  <div className="flex justify-between items-center pt-5 border-t border-gray-200 mt-2">
                    <span className="text-base font-bold text-gray-900">Total ({form.currency_code || "INR"})</span>
                    <span className="text-xl font-bold text-gray-900">{formatCurrency(total, form.currency_code || "INR")}</span>
                  </div>
                </div>
              </div>
